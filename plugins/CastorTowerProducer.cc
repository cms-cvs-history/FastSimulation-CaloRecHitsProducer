...
#include "FastSimulation/CaloRecHitsProducer/plugins/CastorTowerProducer.h"
   ...
   for (size_t i = 0; i < genParticles->size(); ++i) {
	...
	if ( fabs(p.eta()) > 5.2 && fabs(p.eta()) < 6.6) {
		...
		// apply energy smearing with gaussian random generator
   	    	...
   	    	double sigma = sqrt(pow(0.044,2) + pow(0.513/sqrt(p.energy()),2));
		...
		double sigma = sqrt(pow(0.121,2) + pow(1.684/sqrt(p.energy()),2));
   	    	gaus_E = r.Gaus(p.energy(),sigma);
		}
